import styled from 'styled-components';
import { HiChevronLeft, HiChevronRight } from 'react-icons/hi2';
import { useSearchParams } from 'react-router-dom';
import { PAGE_SIZE } from '../utils/constants';

const StyledContainer = styled.div`
    width: 100%;
    display: flex;
    justify-content: space-between;
    align-items: cemter;
`;

const Paragraph = styled.p`
    font-size: 1.4rem;
    margin-left: 0.8rem;

    & span {
        font-weight: 600;
    }
`;

const Buttons = styled.div`
    display: flex;
    gap: 0.6rem;
`;

const PaginationButton = styled.button`
    border: none;
    border-radius: var(--border-radius-sm);
    font-weight: 600;
    font-size: 1.4rem;

    display: flex;
    align-items: center;
    justify-content: center;
    gap: 0.4rem;
    padding: 0.6rem 1.2rem;
    transition: all 0.3s;

    color: ${(props) => (props.active ? 'var(--color-brand-50)' : 'inherit')};
    background-color: ${(props) =>
        props.active ? 'var(--color-brand-600)' : 'var(--color-grey-100)'};

    &:has(span:last-child) {
        padding-left: 0.4rem;
    }

    &:has(span:first-child) {
        padding-right: 0.4rem;
    }

    &svg {
        height: 1.8rem;
        width: 1.8rem;
    }

    &:hover:not(:disabled) {
        background-color: var(--color-brand-600);
        color: var(--color-brand-50);
    }
`;

function Pagination({ count }) {
    const [searchParams, setSearchParams] = useSearchParams();
    const curPage = !searchParams.get('page') ? 1 : Number(searchParams.get('page'));
    const maxPages = Math.ceil(count / PAGE_SIZE);

    const next = () => {
        const next = curPage === maxPages ? curPage : curPage + 1;
        searchParams.set('page', next);
        setSearchParams(searchParams);
    };

    const prev = () => {
        const prev = curPage === 1 ? curPage : curPage - 1;
        searchParams.set('page', prev);
        setSearchParams(searchParams);
    };

    if (maxPages <= 1) return null;

    return (
        <StyledContainer>
            <Paragraph>
                Showing <span>{(curPage - 1) * PAGE_SIZE + 1}</span> to{' '}
                <span>{curPage === maxPages ? count : curPage * PAGE_SIZE}</span> of{' '}
                <span>{count}</span> results
            </Paragraph>
            <Buttons>
                <PaginationButton onClick={() => prev()} disabled={curPage === 1}>
                    <HiChevronLeft /> <span>Previous</span>
                </PaginationButton>
                <PaginationButton onClick={() => next()} disabled={curPage === maxPages}>
                    <HiChevronRight /> <span>Next</span>
                </PaginationButton>
            </Buttons>
        </StyledContainer>
    );
}
export default Pagination;
